...
serve(async () => {
  ...
  const { data: settings } = await sb.from('notification_settings').select('user_id').eq('daily_morning', true)
  const ids = settings?.map((s: any) => s.user_id) ?? []
  if (!ids.length) return new Response('no users')

  const { data: tokens } = await sb.from('push_tokens').select('token').in('user_id', ids)
  if (!tokens?.length) return new Response('no tokens')

  const at = await fcmToken()
  for (const { token } of tokens) {
    try { await push(at, token, '☀️ Buenos días', '¿Qué registraremos hoy?') } catch (_) { /* skip failed token */ }
  }
