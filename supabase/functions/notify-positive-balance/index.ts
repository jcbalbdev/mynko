...
serve(async () => {
  ...
  // Check last 3 complete weeks — runs Sunday 8pm (Mon 1am UTC)
  const now = new Date()
  ...
  const { data: settings } = await sb.from('notification_settings').select('user_id').eq('positive_balance', true)
  ...
  for (const userId of ids) {
    ...
    for (const { start, end } of weeks) {
      const { data: exp } = await sb.from('expenses').select('amount, type').eq('user_id', userId)
        .gte('date', start.toISOString()).lte('date', end.toISOString())

      const income   = (exp ?? []).filter((e: any) => e.type === 'ingreso').reduce((s: number, e: any) => s + Number(e.amount), 0)
      const expenses = (exp ?? []).filter((e: any) => e.type !== 'ingreso').reduce((s: number, e: any) => s + Number(e.amount), 0)
      if (income > expenses && income > 0) positiveWeeks++
    }

    if (positiveWeeks === 3) {
      for (const { token } of userTokens) {
        try { await push(at, token, '✅ Llevas 3 semanas en positivo', 'Tus ingresos superan tus gastos. Buen ritmo, sigue así.') } catch (_) {}
      }
    }
  }
